...
<head>
  ...
  <style>
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .tg{display:inline-flex;align-items:center;cursor:pointer;font-weight:500;font-size:14px;-webkit-user-select:none;user-select:none}
    .tgi{position:absolute;opacity:0;cursor:pointer;height:0;width:0}
    .tk{display:inline-block;height:20px;width:36px;background:var(--s);border-radius:50px;position:relative;vertical-align:middle;margin-right:10px;transition:background .2s}
    .tk:before{position:absolute;content:"";height:14px;width:14px;border-radius:50%;left:3px;top:3px;background:var(--c);transition:transform .2s}
    .tgi:checked+.tk{background:var(--p)}
    .tgi:checked+.tk:before{transform:translateX(16px);background:var(--op)}
    .rw{display:flex;gap:var(--ss)}
  </style>
</head>
<body>
  <div class="wr">
    <div class="brd">
      <div class="brd-i"><span class="mat">monitoring</span></div>
      <div>
        <h1>Price Monitor</h1>
        <div class="sb">极简 · 智能 · 省心</div>
      </div>
    </div>
    
    <div id="warningSection"></div>
    
    <div id="searchSection" class="cd">
      ...
      <div style="display:flex;gap:var(--ss)">
        ...
        <button class="bp" onclick="doSearch()" style="width:56px;flex-shrink:0;padding:0;border-radius:14px"><span class="mat">search</span></button>
      </div>
    ...
    <div class="sh">
      <h2>监控应用</h2>
      <button class="bs" onclick="checkAll()"><span class="mat">refresh</span></button>
    </div>
    <div id="appsContainer"></div>
    
    <div class="cd">
      <div class="sc-h"><span class="mat">add_box</span><h2>添加应用</h2></div>
      <form id="af" onsubmit="addApp(event)">
        <div style="display:grid;gap:var(--ss)">
          <div>
            <div class="lb">Google Play ID</div>
            <input class="in" name="app_id" placeholder="com.flyersoft.moonreaderp" required>
          </div>
          <div>
            <div class="lb">显示名称 <span style="color:var(--m);font-weight:400">（可选）</span></div>
            <input class="in" name="name" placeholder="留空自动获取">
          </div>
          <div>
            <div class="lb">备注</div>
            <input class="in" name="note" placeholder="可写备注信息">
          </div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--ss)">
            <div>
              <div class="lb">降价阈值 (USD)</div>
              <input class="in" name="threshold" type="number" step="0.01" value="6" required>
            </div>
            <div>
              <div class="lb">地区</div>
              <input class="in" name="country" value="us">
            </div>
          </div>
          <div>
            <label class="tg">
              <input type="checkbox" class="tgi" name="monitor_mode" value="change">
              <span class="tk"></span> 价格变动时通知
            </label>
          </div>
        </div>
        <button type="submit" class="bp" style="margin-top:var(--sm)"><span class="mat" style="font-size:20px">add</span>添加监控</button>
      </form>
    </div>
    
    <div class="cd">
      <div class="sh"><h2>通知记录</h2></div>
      <div id="historyContainer"></div>
    </div>
    
    <div class="ft">Cloudflare Workers · Wise Design</div>
  </div>
  
  <!-- Detail Modal -->
  <div id="dv" class="ov" onclick="if(event.target===this)closeDetail()">
    <div class="md" id="detailMd" onclick="event.stopPropagation()">
      <div id="detailContent"></div>
      <div style="margin-top:var(--ss)">
        <div class="lb">降价阈值 (USD)</div>
        <input class="in" id="dtThreshold" type="number" step="0.01" value="6" style="height:40px;font-size:14px">
      </div>
      <div style="margin-top:var(--ss)">
        <div class="lb">地区</div>
        <input class="in" id="dtCountry" value="us" style="height:40px;font-size:14px">
      </div>
      <div style="margin-top:var(--ss)">
        <div class="lb">备注</div>
        <input class="in" id="dtNote" placeholder="可写备注信息" style="height:40px;font-size:14px">
      </div>
      <div style="margin-top:var(--ss)">
        <label class="tg">
          <input type="checkbox" class="tgi" id="dtMonitorMode">
          <span class="tk"></span> 价格变动时通知
        </label>
      </div>
  ...
  <!-- Edit Modal -->
  <div id="ov" class="ov" onclick="if(event.target===this)closeEdit()">
    <div class="md" onclick="event.stopPropagation()">
      <h2>编辑应用</h2>
      <div style="display:grid;gap:var(--ss)">
        <div>
          <div class="lb">显示名称</div>
          <input class="in" id="eName">
        </div>
        <div>
          <div class="lb">备注</div>
          <input class="in" id="eNote" placeholder="可写备注信息">
        </div>
        <div>
          <div class="lb">降价阈值 (USD)</div>
          <input class="in" id="eThreshold" type="number" step="0.01">
        </div>
        <div>
          <div class="lb">地区</div>
          <input class="in" id="eCountry">
        </div>
        <div>
          <label class="tg">
            <input type="checkbox" class="tgi" id="eMonitorMode">
            <span class="tk"></span> 价格变动时通知
          </label>
        </div>
      </div>
      <button class="bp" onclick="saveEdit()" style="margin-top:var(--sm)">保存</button>
    </div>
  </div>
  ...
  <script>
    ...
    function show(m){
      ...
      tt.classList.add("s");
      clearTimeout(ttm);
      ttm=setTimeout(function(){tt.classList.remove("s")},2500)
    }
    ...
    async function api(p,o){
      var r=await fetch(p,Object.assign({},o,{
    ...
    function addApp(e){
      ...
      api("/api/apps",{method:"POST",body:JSON.stringify({
        app_id:f.get("app_id"),
        name:f.get("name")||"",
        threshold:parseFloat(f.get("threshold")),
        country:f.get("country"),
        note:f.get("note")||"",
        monitor_mode:f.get("monitor_mode")?"change":"threshold"})})
       .then(function(){
         show("已添加");
    ...
    function removeApp(id){
      ...
      api("/api/apps",{method:"DELETE",body:JSON.stringify({app_id:id})})
         .then(function(){
           show("已删除");
           setTimeout(function(){location.reload()},800)})
    }

    function editApp(id,n,c,t,nt,mm){
      edId=id;
      document.getElementById("eName").value=n;
      document.getElementById("eThreshold").value=t;
      document.getElementById("eCountry").value=c;
      document.getElementById("eNote").value=nt||"";
      document.getElementById("eMonitorMode").checked=!!mm;
    ...
    function saveEdit(){
      ...
      var n=document.getElementById("eName").value.trim(),
          t=parseFloat(document.getElementById("eThreshold").value),
          c=document.getElementById("eCountry").value.trim(),
          nt=document.getElementById("eNote").value.trim(),
          mm=document.getElementById("eMonitorMode").checked;
      ...
      b.monitor_mode = mm ? "change" : "threshold";
      
      api("/api/apps",{method:"PATCH",body:JSON.stringify(b)})
         .then(function(){
           show("已更新");
           closeEdit();
           setTimeout(function(){location.reload()},800)})
    ...
    async function doSearch(){
      var q=document.getElementById("searchTerm").value.trim();
      if(!q){
        show("请输入关键词");
        return
      }
      ...
      try{
        var d=await api("/api/search?term="+encodeURIComponent(q));
        if(!d.results||d.results.length===0){
          el.innerHTML="<div style='text-align:center;padding:20px;color:var(--m)'>未找到结果</div>";
          return
        }
        ...
        for(var i=0;i<d.results.length;i++){
          var r=d.results[i];
          h+="<div class='sri' onclick='showDetail(\\\\\""+r.appId+"\\\\\",\\\\\""+escJs(r.title)+"\\\\\",\\\\\""+escJs(r.icon||"")+"\\\\\",\\\\\""+escJs(r.priceText||(r.free?"免费":""))+"\\\\\",\\\\\""+escJs(r.developer||"")+"\\\\\",\\\\\""+escJs(r.scoreText||"")+"\\\\")'>";
          if(r.icon){
            h+="<img src='"+r.icon+"' alt='' onerror='this.style.display=\\\"none\\\"'>"
          }
          h+="<div class='srd'><div class='srn'>"+r.title+"</div><div class='sra'>"+r.appId+" - "+r.developer+"</div></div><div class='srp'>"+(r.priceText||(r.free?"免费":""))+"</div></div>"
        }
        
        el.innerHTML="<div class='sr'>"+h+"</div>"
      }catch(e){
        el.innerHTML="<div style='text-align:center;padding:20px;color:var(--m)'>搜索失败</div>"
      }
    }

    // Detail modal functions
    function showDetail(id,title,icon,price,dev,score){
      detailData={id:id,title:title};
      var h="";
      
      h+="<div style='display:flex;align-items:center;gap:var(--sm);margin-bottom:var(--ss)'>";
      if(icon){
        h+="<img src='"+icon+"' alt='' style='width:44px;height:44px;border-radius:10px;flex-shrink:0' onerror='this.style.display=\\\"none\\\"'>";
      }
      h+="<div><div style='font-size:17px;font-weight:700'>"+title+"</div><div style='font-size:11px;color:var(--m);margin-top:1px'>"+id+"</div></div></div>";
      
      h+="<div class='g' style='margin-bottom:0'>";
      h+="<div class='gi' style='padding:10px'><div class='gl'>评分</div><div class='v'><span class='star'>★</span> "+(score||"-")+"</div></div>";
      h+="<div class='gi' style='padding:10px'><div class='gl'>价格</div><div class='v'>"+(price||"-")+"</div></div>";
      h+="</div>";
      
      if(dev){
        h+="<div class='gi' style='padding:10px;margin-top:var(--ss)'><div class='gl'>开发者</div><div class='v'>"+dev+"</div></div>"
      }
    ...
    function addFromDetail(){
      ...
      var t=parseFloat(document.getElementById("dtThreshold").value);
      var c=document.getElementById("dtCountry").value.trim();
      var nt=document.getElementById("dtNote").value.trim();
      var mm=document.getElementById("dtMonitorMode").checked;
      ...
      api("/api/apps",{method:"POST",body:JSON.stringify({
        ...
        country:c||"us",
        note:nt,
        monitor_mode:mm?"change":"threshold"})})
         .then(function(){
           show("已添加");
           closeDetail();
           setTimeout(function(){location.reload()},800)})
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      api("/api/apps")
         .then(function(data){
           ...
           var warnSection = document.getElementById("warningSection");
           if (!data.hasSc3) {
             warnSection.innerHTML = '<div class="w"><span class="mat" style="font-size:16px">warning</span> 未配置通知</div>';
           }
           
           // Handle search section visibility
    ...
    function renderHistory(history) {
      ...
      container.innerHTML = '<div>' + hr + '</div>';
    }
  </script>
</body>
</html>`;
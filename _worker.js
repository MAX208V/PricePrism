...
var SCRIPT = [
  ...
  'function getPriceDisplay(r){if(!r)return"-";if(r.free===true||r.price===0){var a=r.containsAds===true,i=r.inAppPurchases===true||r.offersIAP===true;if(a&&i)return"免费·含广告+内购";if(a)return"免费·含广告";if(i)return"免费·含内购";return"免费"}if(r.price!==undefined&&r.price!==null&&r.price!==0)return"$"+parseFloat(r.price).toFixed(2);return"-"}',
  ...
  'async function doSearch(){var q=document.getElementById("searchTerm").value.trim();if(!q){show("请输入关键词");return}var el=document.getElementById("searchResults");el.textContent="搜索中...";try{var d=await api("/api/search?term="+encodeURIComponent(q));if(!d.results||d.results.length===0){el.innerHTML="<div style=\x27text-align:center;padding:20px;color:var(--m)\x27>未找到结果</div>";return}var h="";for(var i=0;i<d.results.length;i++){var r=d.results[i],pd=getPriceDisplay(r);h+="<div class=\x27sri\x27 onclick=\x27showDetail(\""+r.appId+"\",\""+escJs(r.title)+"\",\""+escJs(r.icon||"")+"\",\""+escJs(pd)+"\",\""+escJs(r.developer||"")+"\",\""+escJs(r.scoreText||"")+"\")\x27>";if(r.icon){h+="<img src=\x27"+r.icon+"\x27 alt=\x27\x27 onerror=\x27this.style.display=\"none\"\x27>"}h+="<div class=\x27srd\x27><div class=\x27srn\x27>"+r.title+"</div><div class=\x27sra\x27>"+r.appId+" - "+r.developer+"</div></div><div class=\x27srp\x27>"+pd+"</div></div>"}el.innerHTML="<div class=\x27sr\x27>"+h+"</div>"}catch(e){el.innerHTML="<div style=\x27text-align:center;padding:20px;color:var(--m)\x27>搜索失败</div>"}}',
  'function escJs(s){if(!s)return"";return s.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\x27/g,"\x27").replace(/"/g,"&quot;")}',
  'function showDetail(id,title,icon,price,dev,score){detailData={id:id,title:title};var h="";h+="<div style=\x27display:flex;align-items:center;gap:var(--sm);margin-bottom:var(--ss)\x27>";if(icon){h+="<img src=\x27"+icon+"\x27 alt=\x27\x27 style=\x27width:44px;height:44px;border-radius:10px;flex-shrink:0\x27 onerror=\x27this.style.display=\"none\"\x27>"}h+="<div><div style=\x27font-size:17px;font-weight:700\x27>"+title+"</div><div style=\x27font-size:11px;color:var(--m);margin-top:1px\x27>"+id+"</div></div></div>";h+="<div class=\x27g\x27 style=\x27margin-bottom:0\x27>";h+="<div class=\x27gi\x27 style=\x27padding:10px\x27><div class=\x27gl\x27>评分</div><div class=\x27v\x27><span class=\x27mat\x27 style=\x27font-size:14px;color:#fbbc04;vertical-align:middle\x27>star</span> "+(score||"-")+"</div></div>";h+="<div class=\x27gi\x27 style=\x27padding:10px\x27><div class=\x27gl\x27>价格</div><div class=\x27v\x27>"+(price||"-")+"</div></div>";h+="</div>";if(dev){h+="<div class=\x27gi\x27 style=\x27padding:10px;margin-top:var(--ss)\x27><div class=\x27gl\x27>开发者</div><div class=\x27v\x27>"+dev+"</div></div>"}document.getElementById("detailContent").innerHTML=h;document.getElementById("dv").classList.add("s")}',

...
function renderHtml(apps, history, hasSc3, hasProxy) {
  try {
    ...
    for (var i = 0; i < apps.length; i++) {
      ...
      var extra = '<div class="gi" style="grid-column:1/3"><div style="display:flex;justify-content:space-between;align-items:flex-start;gap:var(--ss)"><div><div class="gl">评分 / 心愿单</div><div class="v">' + sv + '</div></div>' + (note ? '<div style="text-align:right;flex-shrink:0;max-width:50%"><div class="gl">备注</div><div class="v" style="font-size:12px;color:#856404">' + esc(note) + '</div></div>' : '') + '</div></div>';

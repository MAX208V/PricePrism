...
function renderHtml(apps, history, hasSc3, hasProxy) {
  try {
    ...
    addForm += '<div><label class="tg"><input type="checkbox" class="tgi" name="monitor_mode"><span class="tk"></span> 价格变动时通知</label></div>';
    ...
    out += '</style></head><body><div class="wr"><div class="brd"><div class="brd-i"><span class="mat">monitoring</span></div><div><h1>Price Monitor</h1><div class="sb">极简 - 智能 - 省心</div></div></div>';
    out += warn + searchBox;
    out += '<div class="sh"><span class="st">监控应用</span><button class="bs" onclick="checkAll()"><span class="mat">refresh</span></button></div>';
    out += noApps + addForm;

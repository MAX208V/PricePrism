...
var SCRIPT = [
  ...
  'function addFromDetail(){if(!detailData)return;var btn=document.getElementById("detailAddBtn");var t=parseFloat(document.getElementById("dtThreshold").value);var c=document.getElementById("dtCountry").value.trim();var nt=document.getElementById("dtNote").value.trim();var mm=document.getElementById("dtMonitorModechecked;if(isNaN(t)||t<=0){show("无效阈值");return}btn.disabled=true;btn.textContent="添加中...";api("/api/apps",{method:"POST",body:JSON.stringify({app_id:detailData.id,name:detailData.title,threshold:t,country:c||"us",note:nt||"",monitor_mode:mm?"change":"threshold"})}).then(function(){show("已添加");closeDetail();setTimeout(function(){location.reload()},800)}).catch(function(){btn.disabled=false;btn.textContent="添加监控"})}',
].join("");
...
function renderHtml(apps, history, hasSc3, hasProxy) {
  try {
    ...
    for (var j = 0; j < history.length; j++) {
      var hh = history[j];
      hr += '<div class="hr"><span class="ht">' + fmtUTC(hh.time) + '</span><span class="hn">' + esc(hh.name) + '</span><span class="hp">$' + hh.price + '</span><span class="bg hb' + (hh.notified ? "" : "") + '">' + (hh.notified ? "已通知" : "跳过") + '</span></div>';
    }
    ...
    out += '.rn{display:inline-flex;align-items:center;justify-content:center;height:34px;padding:6px 20px;border-radius:var(--rx);font-family:var(--f);font-size:13px;font-weight:600;cursor:pointer;border:none;gap:4px;background:var(--s);color:var(--k)}.rn:hover{background:var(--pp);color:#163300}.rn .mat{font-size:16px}';
    out += '.st{display:inline-flex;align-items:center;padding:4px 16px;border-radius:var(--rp);background:var(--p);color:var(--op);font-size:14px;font-weight:700;height:30px;gap:4px}.sc-h{display:flex;align-items:center;gap:var(--ss);margin-bottom:var(--ss)}.sc-h .mat{font-size:22px;color:var(--p)}.sc-h h2{font-size:18px;font-weight:700;letter-spacing:-.02em}.ac{background:var(--c);border-radius:var(--rx);box-shadow:0 4px 24px rgba(14,15,12,.06);overflow:hidden}.ach{display:flex;align-items:center;gap:var(--sm);padding:var(--sl) var(--sl) var(--ss)}.aci-icon{width:36px;height:36px;border-radius:8px;flex-shrink:0}.acn{display:flex;flex-direction:column;gap:2px;min-width:0;flex:1}.act{font-size:17px;font-weight:700;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.aci{font-size:11px;color:var(--m);font-weight:500;word-break:break-all}.acb{padding:0 var(--sl) var(--sl)}';
    ...
    out += '.in{width:100%;height:50px;background:var(--c);border:2px solid var(--k);border-radius:var(--rm);padding:0 var(--sl);font-family:var(--f);font-size:16px;color:var(--k);outline:none}.in:focus{border-color:var(--p);box-shadow:0 0 0 3px rgba(159,232,112,.2)}.lb{font-size:10px;font-weight:700;text-transform:uppercase;color:var(--b);margin-bottom:6px;letter-spacing:.03em}.cd{background:var(--c);border-radius:var(--rx);box-shadow:0 4px 24px rgba(14,15,12,.06);padding:var(--sl)}.sh{display:flex;align-items:center;justify-content:space-between;margin-bottom:var(--ss)}';
    ...
    out += warn + searchBox;
    out += '<div class="sh"><button class="rn" onclick="checkAll()"><span class="mat" style="font-size:16px">refresh</span> 刷新</button></div>';
    out += noApps + addForm;
    out += '<div class="cd"><div class="sh"><span class="st">通知记录</span></div>' + (history.length ? '<div>' + hr + '</div>' : '<div style="text-align:center;padding:20px;color:var(--m);font-weight:500;font-size:14px">暂无记录</div>') + '</div>';

...
export function renderHtml(apps, history, hasSc3, hasProxy) {
  ...
  for (var j = 0; j < history.length; j++) {
    var h = history[j];
    hr += '<div class="hr"><span class="ht">' + fmtUTC(h.time) + '</span><span class="hn">' + esc(h.name) + '</span><span class="hp">$' + h.price + '</span><span class="bg hb' + (h.notified ? "" : " gy") + '">' + (h.notified ? "已通知" : "跳过") + '</span></div>';
  }

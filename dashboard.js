...
var SCRIPT = [
  ...
  'function getPriceDisplay(a){if(!a)return"N/A";if(a.free===true||a.price===0){var d=a.containsAds||a.hasAds,p=a.inAppPurchases||a.offersIAP||a.hasIAP;if(d&&p)return"免费 · 含广告+内购";if(d)return"免费 · 含广告";if(p)return"免费 · 含内购";return"免费"}if(a.price!==undefined&&a.price!==null&&a.price!==0)return"$"+parseFloat(a.price).toFixed(2);return"-"}',
  'function escJs(s){if(!s)return"";return String(s).replace(/\\\\\\\\/g,"\\\\\\\\\\\\\\\\").replace(/\\"/g,"\\\\\\\\\\\\\\\"").replace(/</g,"\\\\\\\\<").replace(/>/g,"\\\\\\\\>")}',
  'async function doSearch(){var q=document.getElementById("searchTerm").value.trim();if(!q){show("请输入关键词");return}var el=document.getElementById("searchResults");el.textContent="搜索中...";try{var d=await api("/api/search?term="+encodeURIComponent(q));if(!d.results||d.results.length===0){el.innerHTML="<div style=\\'text-align:center;padding:20px;color:var(--m)\\'>未找到结果</div>";return}var h="";for(var i=0;i<d.results.length;i++){var r=d.results[i],pd=getPriceDisplay(r);h+="<div class=\\'sri\\' onclick=\\'showDetail(\\""+r.appId+"\\",\\""+escJs(r.title)+"\\",\\""+escJs(r.icon||"")+"\\",\\""+escJs(pd)+"\\",\\""+escJs(r.developer||"")+"\\",\\""+escJs(r.scoreText||"")+"\\")\\'>";if(r.icon)h+="<img src=\\'"+r.icon+"\\' alt=\\'\\' onerror=\\'this.style.display=\\"none\\"\\'>";h+="<div class=\\'srd\\'><div class=\\'srn\\'>"+r.title+"</div><div class=\\'sra\\'>"+r.appId+" - "+r.developer+"</div></div><div class=\\'srp\\'>"+pd+"</div></div>"}el.innerHTML="<div class=\\'sr\\'>"+h+"</div>"}catch(e){el.innerHTML="<div style=\\'text-align:center;padding:20px;color:var(--m)\\'>搜索失败</div>"}}',
  ...
  'function addFromDetail(){if(!detailData)return;var btn=document.getElementById("detailAddBtn"),t=parseFloat(document.getElementById("dtThreshold").value),c=document.getElementById("dtCountry").value.trim();if(isNaN(t)||t<=0){show("无效阈值");return}btn.disabled=true;btn.textContent="添加中...";api("/api/apps",{method:"POST",body:JSON.stringify({app_id:detailData.id,name:detailData.title,threshold:t,country:c||"us"})}).then(function(){show("已添加");closeDetail();setTimeout(function(){location.reload()},800)}).catch(function(){btn.disabled=false;btn.textContent="添加监控"})}',
].join("");

...
var SCRIPT = [
  ...
  'function getPriceDisplay(app){if(!app)return"N/A";if(app.free===true||app.price===0){var ads=app.containsAds||app.hasAds;var iap=app.inAppPurchases||app.offersIAP||app.hasIAP;if(ads&&iap)return"免费 · 含广告+内购";if(ads)return"免费 · 含广告";if(iap)return"免费 · 含内购";return"免费"}if(app.price!==undefined&&app.price!==null&&app.price!==0){return"$"+parseFloat(app.price).toFixed(2)}return"-"}',
  'async function doSearch(){var q=document.getElementById("searchTerm").value.trim();if(!q){show("请输入关键词");return}var el=document.getElementById("searchResults");el.textContent="搜索中...";try{var d=await api("/api/search?term="+encodeURIComponent(q));if(!d.results||d.results.length===0){el.innerHTML="<div style=\'text-align:center;padding:20px;color:var(--m)\'>未找到结果</div>";return}var h="";for(var i=0;i<d.results.length;i++){var r=d.results[i];var priceDisplay=getPriceDisplay(r);h+="<div class=\'sri\' onclick=\'showDetail(\\""+r.appId+"\\",\\""+escJs(r.title)+"\\",\\""+escJs(r.icon||"")+"\\",\\""+escJs(priceDisplay)+"\\",\\""+escJs(r.developer||"")+"\\",\\""+escJs(r.scoreText||"")+"\\")\'>";if(r.icon){h+="<img src=\'"+r.icon+"\' alt=\'\' onerror=\'this.style.display=\\"none\\"\'>"}h+="<div class=\'srd\'><div class=\'srn\'>"+r.title+"</div><div class=\'sra\'>"+r.appId+" - "+r.developer+"</div></div><div class=\'srp\'>"+priceDisplay+"</div></div>"}el.innerHTML="<div class=\'sr\'>"+h+"</div>"}catch(e){el.innerHTML="<div style=\\'text-align:center;padding:20px;color:var(--m)\\'>搜索失败</div>"}}',

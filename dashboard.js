...
export function renderHtml(apps, history, hasSc3) {
  ...
  for (const a of apps) {
    ...
    const ts = s.last_checked_at ? new Date(s.last_checked_at).toLocaleString("zh-CN", { hour12: false }) : "-";
    const lo = p !== undefined && p > 0 && p < a.threshold;
    const ns = s.last_notified_at ? new Date(s.last_notified_at).toLocaleString("zh-CN", { hour12: false }) : "-";
  ...
  for (const h of history) {
    const t = new Date(h.time).toLocaleString("zh-CN", { month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", hour12: false });
    historyRows += '<div class="hr"><span class="ht">' + t + '</span><span class="hn">' + esc(h.name) + '</span><span class="hp">$' + h.price + '</span><span class="bg hb' + (h.notified ? "" : " gy") + '">' + (h.notified ? "已通知" : "跳过") + '</span></div>';
  }

  const noAppsHtml = apps.length === 0 ? '<div class="cd" style="text-align:center;padding:32px;color:var(--m);font-weight:500;font-size:14px">暂无监控应用</div>' : cards;
  const warnHtml = !hasSc3 ? '<div class="w"><span class="mat" style="font-size:16px">warning</span> 未配置通知</div>' : "";
  ...
  return '<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover,user-scalable=no"><title>Price Monitor</title><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;900&display=swap" rel="stylesheet"><link href="https://fonts.googleapis.com/css2?family=Material+Symbols+Rounded:opsz,wght,FILL,GRAD@24,400,0,0" rel="stylesheet"><style>'
    ...
    + '.bs{display:inline-flex;align-items:center;justify-content-radius:var(--rp);fontborder:none;background:var(--s);color:var(--k)}'
    + '.bs:hover{background:var(--pp);color:#163300}'
    + '.bs .mat{font-size:18px}'
    + '.bs.br .mat{color:var(--neg)}'
    + '.bp{display:inline-flex;align-items:center;justify-content:center;height:46px;padding:var(--sm) var(--sxl);border-radius:var(--rp);font-family:var(--f);font-size:16px;font-weight:600;cursor:pointer;border:none;gap:6px;background:var););   background'
:background:var(--c);border:2px solid var(--k);border-radius:var(--rm);padding:0 var(--sl);font-family:var(--f);font-size:16px;color:var(--k);outline:none +{bordervar); px rgba(159, +-size:10px;font-weight:700;text-transform:uppercase;color:var(--b);margin-bottom:6px;letter-spacing:.03em}'
    ...
    + '<div class="cd"><div class="sh"><h2>通知记录</h2></div>' + (history.length ? '<div>' + historyRows + '</div>' : '<div style="text-align:center;padding:20px;color:var(--m);font-weight:500;font-size:14px">暂无记录</div> '</>'

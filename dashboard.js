...
export function renderHtml(apps, history, hasSc3, hasProxy) {
  ...
  for (const a of apps) {
    ...
    cards += '<div class="ac">' + head
      ...
      + '<button class="bs br" onclick="removeApp(' + "'" + esc(a.id) + "'" + ')"><span class="mat">delete</span></button></div></div></div>';
  }

  let hr = "";
  for (const h of history) {
    const t = fmtUTC(h.time);
  ...
  const noApps = apps.length === 0
    ? '<div class="cd" style="text-align:center;padding:32px;color:var(--m);font-weight:500;font-size:14px">暂无监控应用</div>'
    : cards;
  const warn = !hasSc3
  ...
  s.push('function getPriceDisplay(r){if(!r)return"";if(r.free===true||r.price===0){var a=r.containsAds===true,i=r.inAppPurchases===true||r.offersIAP===true;if(a&&i)return"免费·含广告+内购";if(a)return"免费·含广告";if(i)return"免费·含内购";return"免费"}if(r.price!==undefined&&r.price!==null&&r.price!==0)return"$"+parseFloat(r.price).toFixed(2);return"-"}');
  ...
  s.push('async function doSearch(){var q=document.getElementById("searchTerm").value.trim();if(!q){show("请输入关键词");return}var el=document.getElementById("searchResults");el.textContent="搜索中...";try{var d=await api("/api/search?term="+encodeURIComponent(q));if(!d.results||d.results.length===0){el.innerHTML="<div style=\'text-align:center;padding:20px;color:var(--m)\'>未找到结果</div>";return}var h="";for(var i=0;i<d.results.length;i++){var r=d.results[i],pd=getPriceDisplay(r);h+="<div class=\'sri\' onclick=\'pickApp(\\""+r.appId+"\\",\\""+r.title.replace(/"/g,"&quot;")+"\\")\'>";if(r.icon){h+="<img src=\'"+r.icon+"\' alt=\'\' onerror=\'this.style.display=\\"none\\"\'>"}h+="<div class=\'srd\'><div class=\'srn\'>"+r.title+"</div><div class=\'sra\'>"+r.appId+" - "+r.developer+"</div></div><div class=\'srp\'>"+pd+"</div></div>"}el.innerHTML="<div class=\'sr\'>"+h+"</div>"}catch(e){el.innerHTML="<div style=\'text-align:center;padding:20px;color:var(--m)\'>搜索失败</div>"}}');

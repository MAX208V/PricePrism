/**
 * Dashboard module for rendering the main admin panel
 */
...
export function renderDashboardHtml(apps, history, hasSc3, hasProxy) {
  var cards = "";
  ...
  addForm += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--ss)"><div><div class="lb">降价阈值 (USD)</div><input class="in" name="threshold" type="number" step="0.01" value="6" required></div><div><div class="lb">地区</div><input class="in" name="country" value="us"></div></div>';
  addForm += '<div><label class="tg"><input type="checkbox" class="tgi" name="monitor_mode" value="change"><span class="tk"></span> 价格变动时通知</label></div></div>';
  ...
  out += '<style>';
  out += ':root{--p:#9fe870;--op:#0e0f0c;--pa:#cdffad;--pp:#e2f6d5;--k:#0e0f0c;--b:#454745;--m:#868685;--c:#fff;--s:#e8ebe6;--pos:#2ead4b;--neg:#d03238;--rx:24px;--rm:12px;--rp:9999px;--ss:8px;--sm:12px;--sl:16px;--sxl:24px;--f:Inter,sans-serif}';
  ...
  out += '.tg{display:inline-flex;align-items:center;cursor:pointer;font-weight:500;font-size:14px;-webkit-user-select:none;user-select:none}';
  out += '.tgi{position:absolute;opacity:0;cursor:pointer;height:0;width:0}';
  out += '.tk{display:inline-block;height:20px;width:36px;background:var(--s);border-radius:50px;position:relative;vertical-align:middle;margin-right:10px;transition:background .2s}';
  out += '.tk:before{position:absolute;content:"";height:14px;width:14px;border-radius:50%;left:3px;top:3px;background:var(--c);transition:transform .2s}';
  out += '.tgi:checked+.tk{background:var(--p)}';
  out += '.tgi:checked+.tk:before{transform:translateX(16px);background:var(--op)}';
  out += '.rw{display:flex;gap:var(--ss)}';
  ...
  // Detail modal
  out += '<div id="dv" class="ov" onclick="if(event.target===this)closeDetail()"><div class="md" id="detailMd" onclick="event.stopPropagation()">';
  out += '<div id="detailContent"></div>';
  out += '<div style="margin-top:var(--ss)"><div class="lb">降价阈值 (USD)</div><input class="in" id="dtThreshold" type="number" step="0.01" value="6" style="height:40px;font-size:14px"></div>';
  out += '<div style="margin-top:var(--ss)"><div class="lb">地区</div><input class="in" id="dtCountry" value="us" style="height:40px;font-size:14px"></div>';
  out += '<div style="margin-top:var(--ss)"><div class="lb">备注</div><input class="in" id="dtNote" placeholder="可写备注信息" style="height:40px;font-size:14px"></div>';
  out += '<div style="margin-top:var(--ss)"><label class="tg"><input type="checkbox" class="tgi" id="dtMonitorMode"><span class="tk"></span> 价格变动时通知</label></div>';
  out += '<button class="bp" onclick="addFromDetail()" id="detailAddBtn" style="margin-top:var(--sm);height:40px;font-size:14px">添加监控</button>';
  out += '</div></div>';
  
  // Edit modal
  out += '<div id="ov" class="ov" onclick="if(event.target===this)closeEdit()"><div class="md" onclick="event.stopPropagation()"><h2>编辑应用</h2><div style="display:grid;gap:var(--ss)"><div><div class="lb">显示名称</div><input class="in" id="eName"></div><div><div class="lb">备注</div><input class="in" id="eNote" placeholder="可写备注信息"></div><div><div class="lb">降价阈值 (USD)</div><input class="in" id="eThreshold" type="number" step="0.01"></div><div><div class="lb">地区</div><input class="in" id="eCountry"></div><div><label class="tg"><input type="checkbox" class="tgi" id="eMonitorMode"><span class="tk"></span> 价格变动时通知</label></div></div><button class="bp" onclick="saveEdit()" style="margin-top:var(--sm)">保存</button></div></div>';
  
  out += '<div id="tt" class="tt"></div>';
  
  // Scripts
  out += '<script>';
  out += 'var tt=document.getElementById("tt"),ttm,edId=null,detailData=null;';
  out += 'function show(m){tt.textContent=m;tt.classList.add("s");clearTimeout(ttm);ttm=setTimeout(function(){tt.classList.remove("s")},2500)}';
  out += 'async function api(p,o){var r=await fetch(p,Object.assign({},o,{headers:{"Content-Type":"application/json"}})),d=await r.json();if(!r.ok){show(d.error||"请求失败");throw new Error(d.error)}return d}';
  out += 'function addApp(e){e.preventDefault();var f=new FormData(e.target);api("/api/apps",{method:"POST",body:JSON.stringify({app_id:f.get("app_id"),name:f.get("name")||"",threshold:parseFloat(f.get("threshold")),country:f.get("country"),note:f.get("note")||"",monitor_mode:f.get("monitor_mode")?"change":"threshold"})}).then(function(){show("已添加");setTimeout(function(){location.reload()},800)})}';
  out += 'function removeApp(id){if(!confirm("确认删除？"))return;api("/api/apps",{method:"DELETE",body:JSON.stringify({app_id:id})}).then(function(){show("已删除");setTimeout(function(){location.reload()},800)})}';
  out += 'function editApp(id,n,c,t,nt,mm){edId=id;document.getElementById("eName").value=n;document.getElementById("eThreshold").value=t;document.getElementById("eCountry").value=c;document.getElementById("eNote").value=nt||"";document.getElementById("eMonitorMode").checked=!!mm;document.getElementById("ov").classList.add("s")}';
  out += 'function closeEdit(){edId=null;document.getElementById("ov").classList.remove("s")}';
  out += 'function saveEdit(){if(!edId)return;var n=document.getElementById("eName").value.trim(),t=parseFloat(document.getElementById("eThreshold").value),c=document.getElementById("eCountry").value.trim(),nt=document.getElementById("eNote").value.trim(),mm=document.getElementById("eMonitorMode").checked;if(!n){show("名称不能为空");return}if(isNaN(t)||t<=0){show("无效阈值");return}var b={app_id:edId,name:n,threshold:t,country:c};if(nt){b.note=nt}b.monitor_mode=mm?"change":"threshold";api("/api/apps",{method:"PATCH",body:JSON.stringify(b)}).then(function(){show("已更新");closeEdit();setTimeout(function(){location.reload()},800)})}';
  out += 'function checkAll(){show("正在检查...");api("/api/check").then(function(){show("检查完成");setTimeout(function(){location.reload()},1500)})}';
  out += 'async function doSearch(){var q=document.getElementById("searchTerm").value.trim();if(!q){show("请输入关键词");return}var el=document.getElementById("searchResults");el.textContent="搜索中...";try{var d=await api("/api/search?term="+encodeURIComponent(q));if(!d.results||d.results.length===0){el.innerHTML="<div style=\'text-align:center;padding:20px;color:var(--m)\'>未找到结果</div>";return}var h="";for(var i=0;i<d.results.length;i++){var r=d.results[i];h+="<div class=\'sri\' onclick=\'showDetail(\\\\"'+r.appId+'\\\\",\\\\"'+escJs(r.title)+'\\\\",\\\\"'+escJs(r.icon||"")+'\\\\",\\\\"'+escJs(r.priceText||(r.free?"免费":""))+'\\\\",\\\\"'+escJs(r.developer||"")+'\\\\",\\\\"'+escJs(r.scoreText||"")+'\\\\")\'>"+"<img src=\'"+r.icon+"\' alt=\'\' onerror=\'this.style.display=\\\"none\\\"\'><div class=\'srd\'><div class=\'srn\'>"+r.title+"</div><div class=\'sra\'>"+r.appId+"</div></div><div class=\'srp\'>"+(r.priceText||(r.free?"免费":""))+"</div></div>"}el.innerHTML="<div class=\'sr\'>"+h+"</div>"}catch(e){el.innerHTML="<div style=\'text-align:center;padding:20px;color:var(--m)\'>搜索失败</div>"}}';
  out += 'function escJs(s){if(!s)return "";return s.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\'/g,"\\\\\'").replace(/"/g,"&quot;")}';
  out += 'function showDetail(id,title,icon,price,dev,score){detailData={id:id,title:title};var h="";h+="<div style=\'display:flex;align-items:center;gap:var(--sm);margin-bottom:var(--ss)\'><img src=\'"+icon+"\' alt=\'\' style=\'width:44px;height:44px;border-radius:10px;flex-shrink:0\' onerror=\'this.style.display=\\\"none\\\"\'><div><div style=\'font-size:17px;font-weight:700\'>"+title+"</div><div style=\'font-size:11px;color:var(--m);margin-top:1px\'>"+id+"</div></div></div><div class=\'g\' style=\'margin-bottom:0\'><div class=\'gi\' style=\'padding:10px\'><div class=\'gl\'>评分</div><div class=\'v\'><span class=\'star\'>★</span> "+(score||"-")+"</div></div><div class=\'gi\' style=\'padding:10px\'><div class=\'gl\'>价格</div><div class=\'v\'>"+(price||"-")+"</div></div></div>"+(dev?"<div class=\'gi\' style=\'padding:10px;margin-top:var(--ss)\'><div class=\'gl\'>开发者</div><div class=\'v\'>"+dev+"</div></div>":"");document.getElementById("detailContent").innerHTML=h;document.getElementById("dv").classList.add("s")}';
  out += 'function closeDetail(){detailData=null;document.getElementById("dv").classList.remove("s")}';
  out += 'function addFromDetail(){if(!detailData)return;var btn=document.getElementById("detailAddBtn");var t=parseFloat(document.getElementById("dtThreshold").value);var c=document.getElementById("dtCountry").value.trim();var nt=document.getElementById("dtNote").value.trim();var mm=document.getElementById("dtMonitorMode").checked;if(isNaN(t)||t<=0){show("无效阈值");return}btn.disabled=true;btn.textContent="添加中...";api("/api/apps",{method:"POST",body:JSON.stringify({app_id:detailData.id,name:detailData.title,threshold:t,country:c||"us",note:nt,monitor_mode:mm?"change":"threshold"})}).then(function(){show("已添加");closeDetail();setTimeout(function(){location.reload()},800)}).catch(function(){btn.disabled=false;btn.textContent="添加监控"})}';

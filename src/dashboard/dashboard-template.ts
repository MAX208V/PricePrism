// 从 dashboard.js 提取的 HTML 模板和渲染函数
import { fmtUTC, esc } from './dashboard-utils';

// 详细对话框模板
const DTL = '<div id="dv" class="ov" onclick="if(event.target===this)closeDetail()"><div class="md" id="detailMd" onclick="event.stopPropagation()">' +
...
// 编辑对话框模板
const EDT = '<div id="ov" class="ov" onclick="if(event.target===this)closeEdit()"><div class="md" onclick="event.stopPropagation()"><h2>编辑应用</h2><div style="display:grid;gap:var(--ss)"><div><div class="lb">显示名称</div><input class="in" id="eName"></div><div><div class="lb">备注</div><input class="in" id="eNote" placeholder="可写备注信息"></div><div><div class="lb">降价阈值 (USD)</div><input class="in" id="eThreshold" type="number" step="0.01"></div><div><div class="lb">地区</div><input class="in" id="eCountry"></div><div><label class="tg"><input type="checkbox" class="tgi" id="eMonitorMode"><span class="tk"></span> 价格变动时通知</label></div></div><button class="bp" onclick="saveEdit()" style="margin-top:var(--sm)">保存</button></div></div>';
...
export function renderHtml(apps: any[], history: any[], hasSc3: boolean, hasProxy: boolean): string {
  try {
    let cards = "";
    for (let i = 0; i < apps.length; i++) {
      const a = apps[i], st = a.status || {};
      const p = st.last_checked_price, ps = p !== undefined ? "$" + p : "-";
      const ts = fmtUTC(st.last_checked_at), ns = fmtUTC(st.last_notified_at);
      const cm = a.monitor_mode === "change";
      const lo = !cm && p !== undefined && p > 0 && p < a.threshold;
      const icon = st.icon || "", score = st.scoreText || "", ratings = st.ratings || "", note = a.note || "", dev = st.developer || "";
      const rn = parseInt(ratings, 10), rd = !isNaN(rn) && rn >= 1000 ? (rn / 1000).toFixed(1).replace(/\.0$/, "") + "k" : ratings;
      ...
      const sv = (score ? '<span class="mat" style="font-size:14px;color:#fbbc04;vertical-align:middle">star</span> ' + esc(score) + ' ' : "") + (rd ? '<span style="color:var(--m)">|</span> <span class="mat" style="font-size:14px;color:var(--m);vertical-align:middle">bookmark_border</span> ' + esc(rd) : "");
      let extra;
      if (note) {
    ...
    const searchBox = hasProxy ? '<div class="cd" id="searchSection"><div class="sh"><span class="st"><span class="mat">monitoring</span>监控价格</span></div><div style="display:flex;gap:var(--ss);margin-top:var(--ss)"><input class="in" id="searchTerm" placeholder="输入关键词搜索 Google Play..." style="flex:1" onkeydown="if(event.key===\'Enter\'){event.preventDefault();doSearch()}"><button class="bp" onclick="doSearch()" style="width:56px;flex-shrink:0;padding:0"><span class="mat">search</span></button></div><div id="searchResults"></div></div>' : "";
    
    let addForm = '<div class="cd"><div class="sc-h"><span class="mat">add_box</span><h2>添加应用</h2></div>';
    ...
    let out = '<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover,user-scalable=no"><title>Price Monitor</title><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;900&display=swap" rel="stylesheet"><link href="https://fonts.googleapis.com/css2?family=Material+Symbols+Rounded:opsz,wght,FILL,GRAD@24,400,0,0" rel="stylesheet"><style>';

...
var SCRIPT = [
  ...
  'function saveEdit(){if(!edId)return;var n=document.getElementById("eName").value.trim(),t=parseFloat(document.getElementById("eThreshold").value),c=document.getElementById("eCountry").value.trim(),nt=document.getElementById("eNote").value.trim(),mm=document.getElementById("eMonitorMode").checked;if(!n){show("名称不能为空");return}if(isNaN(t)||t<=0){show("无效阈值");return}api("/api/apps",{method:"PATCH",body:JSON.stringify({app_id:edId,name:n,threshold:t,country:c,note:nt,monitor_mode:mm?"change":"threshold"})}).then(function(){show("已更新");closeEdit();setTimeout(function(){location.reload()},800)})}',
...
export function renderHtml(apps, history, hasSc3, hasProxy) {
  ...
  addForm += '<div style="margin-top:10px"><label class="tg"><input type="checkbox" class="tgi" name="monitor_mode"><span class="tk"></span> 价格变动时通知</label></div>';
  ...
  out += '.tg{display:inline-flex;align-items:center;cursor:pointer;font-size:14px;-webkit-user-select:none;-moz-user-select:none;user-select:none}.tgi{position:absolute;opacity:0;cursor:pointer;height:0;width:0}.tk{display:inline-block;width:36px;height:20px;background:var(--s);border-radius:999px;position:relative;transition:background .2s}.tk:before{position:absolute;content:"";height:16px;width:16px;left:2px;bottom:2px;background:var(--c);border-radius:50%;transition:transform .2s}.tgi:checked+.tk{background:var(--p)}.tgi:checked+.tk:before{transform:translateX(16px)}.tgi:focus+.tk{box-shadow:0 0 0 3px rgba(159,232,112,.2)}';

...
function fmtUTC(iso) {
  ...
  "function generateSearchResultHTML(r){var h="";h+="<div class=\'sri\' onclick=\'showDetail(\\\""+escJs(r.appId)+"\\\",\\\""+escJs(r.title)+"\\\",\\\""+escJs(r.icon||""")+"\\\",\\\""+escJs(r.priceText||(r.free?"免费":""))+"\\\",\\\""+escJs(r.developer||""")+"\\\",\\\""+escJs(r.scoreText||""")+"\\\")\'>";if(r.icon){h+="<img src=\'"+r.icon+"\' alt=\'\'\'>"}h+="<div class=\'srd\'><div class=\'srn\'>"+r.title+"</div><div class=\'sra\'>"+r.appId+" - "+r.developer+"</div></div><div class=\'srp\'>"+(r.priceText||(r.free?"免费":""))+"</div></div>";return h;}function doSearchCallback(resultsElement,resultsData){try{var h="";for(var i=0;i<resultsData.length;i++){var r=resultsData[i];h+=generateSearchResultHTML(r);}resultsElement.innerHTML="<div class=\'sr\'>"+h+"</div>";}catch(e){resultsElement.innerHTML="<div style=\'text-align:center;padding:20px;color:var(--m)\'>搜索失败</div>";}}",
